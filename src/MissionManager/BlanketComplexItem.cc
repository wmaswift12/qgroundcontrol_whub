 ...
 ****************************************************************************/
 ...
 QGC_LOGGING_CATEGORY(BlanketComplexItemLog, "BlanketComplexItemLog")

 const QString BlanketComplexItem::name(BlanketComplexItem::tr("Blanket"));
 
 BlanketComplexItem::BlanketComplexItem(PlanMasterController* masterController, bool flyView, const QString& kmlOrShpFile)
     : SprayStyleBlanketItem     (masterController, flyView, settingsGroup)
     , _metaDataMap              (FactMetaData::createMapFromJsonFile(QStringLiteral(":/json/Blanket.SettingsGroup.json"), this))
 ...
 {
     _editorQml = "qrc:/qml/BlanketItemEditor.qml";
     ...
     if (_sprayCalc.isManualCamera() || !_sprayCalc.valueSetIsDistance()->rawValue().toBool()) {
         _sprayCalc.distanceToSurface()->setRawValue(SettingsManager::instance()->appSettings()->defaultMissionItemAltitude()->rawValue());
     }
 
     connect(&_gridAngleFact,            &Fact::valueChanged,                        this, &BlanketComplexItem::_setDirty);
     connect(&_flyAlternateTransectsFact,&Fact::valueChanged,                        this, &BlanketComplexItem::_setDirty);
     connect(&_splitConcavePolygonsFact, &Fact::valueChanged,                        this, &BlanketComplexItem::_setDirty);
     connect(this,                       &BlanketComplexItem::refly90DegreesChanged,  this, &BlanketComplexItem::_setDirty);
 
     connect(&_gridAngleFact,            &Fact::valueChanged,                        this, &BlanketComplexItem::_rebuildTransects);
     connect(&_flyAlternateTransectsFact,&Fact::valueChanged,                        this, &BlanketComplexItem::_rebuildTransects);
     connect(&_splitConcavePolygonsFact, &Fact::valueChanged,                        this, &BlanketComplexItem::_rebuildTransects);
     connect(this,                       &BlanketComplexItem::refly90DegreesChanged,  this, &BlanketComplexItem::_rebuildTransects);
 
     connect(&_surveyAreaPolygon,        &QGCMapPolygon::isValidChanged,             this, &BlanketComplexItem::_updateWizardMode);
     connect(&_surveyAreaPolygon,        &QGCMapPolygon::traceModeChanged,           this, &BlanketComplexItem::_updateWizardMode);
 ...
 void BlanketComplexItem::save(QJsonArray&  planItems)
 {
 ...
 void BlanketComplexItem::savePreset(const QString& name)
 {
     QJsonObject saveObject;
 ...
 void BlanketComplexItem::_saveCommon(QJsonObject& saveObject)
 {
     SprayStyleBlanketItem::_save(saveObject);
 
     saveObject[JsonHelper::jsonVersionKey] =                    5;
     saveObject[VisualMissionItem::jsonTypeKey] =                VisualMissionItem::jsonTypeComplexItemValue;
     saveObject[ComplexMissionItem::jsonComplexItemTypeKey] =    jsonBlanketComplexItemTypeValue;
 ...
 void BlanketComplexItem::loadPreset(const QString& name)
 {
     QString errorString;
 ...
 bool BlanketComplexItem::load(const QJsonObject& complexObject, int sequenceNumber, QString& errorString)
 {
     ...
     if (version == 4 || version == 5) {
        qCDebug(BlanketComplexItemLog) << "Version 4 or 5, go to loadV4V5 TransectComplex"; 
         ...
         if (version == 2) {
            qCDebug(BlanketComplexItemLog) << "Version 2, changing to ver. 3"; 
            // Convert to v3
             if (v3ComplexObject.contains(VisualMissionItem::jsonTypeKey) && v3ComplexObject[VisualMissionItem::jsonTypeKey].toString() == QStringLiteral("survey")) {
                 ...
                 v3ComplexObject[ComplexMissionItem::jsonComplexItemTypeKey] = jsonBlanketComplexItemTypeValue;
             }
         }
         if (!_loadV3(complexObject, sequenceNumber, errorString)) {
             qCDebug(BlanketComplexItemLog) << "Going to loadV3";
             return false;
         }
 ...
 bool BlanketComplexItem::_loadV4V5(const QJsonObject& complexObject, int sequenceNumber, QString& errorString, int version, bool forPresets)
 {
     ...
     if (itemType != VisualMissionItem::jsonTypeComplexItemValue || complexType != jsonBlanketComplexItemTypeValue) {
     ...
     qCDebug(BlanketComplexItemLog) << "Load the SprayStyleBlanketItem file to be continued";
     if (!SprayStyleBlanketItem::_load(complexObject, forPresets, errorString)) {
         _ignoreRecalc = false;
         return false;
     }
     ...
     _ignoreRecalc = false;
     qCDebug(BlanketComplexItemLog) << "Loading Survey items finished";
     return true;
 }
 
 bool BlanketComplexItem::_loadV3(const QJsonObject& complexObject, int sequenceNumber, QString& errorString)
 {
     ...
     QString itemType = complexObject[VisualMissionItem::jsonTypeKey].toString();
     QString complexType = complexObject[ComplexMissionItem::jsonComplexItemTypeKey].toString();
     if (itemType != VisualMissionItem::jsonTypeComplexItemValue || complexType != jsonV3BlanketComplexItemTypeValue) {
         errorString = tr("%1 does not support loading this complex mission item type: %2:%3").arg(qgcApp()->applicationName()).arg(itemType).arg(complexType);
         return false;
     }
     ...
     _sprayCalc.valueSetIsDistance()->setRawValue   (complexObject[_jsonV3FixedValueIsAltitudeKey].toBool(true));
     _sprayCalc.setDistanceMode(complexObject[_jsonV3GridAltitudeRelativeKey].toBool(true) ? QGroundControlQmlGlobal::AltitudeModeRelative : QGroundControlQmlGlobal::AltitudeModeAbsolute);
     ...
     _sprayCalc.distanceToSurface()->setRawValue        (gridObject[_jsonV3GridAltitudeKey].toDouble());
     _sprayCalc.adjustedFootprintSide()->setRawValue    (gridObject[_jsonV3GridSpacingKey].toDouble());
     _sprayCalc.adjustedFootprintFrontal()->setRawValue (complexObject[_jsonV3CameraTriggerDistanceKey].toDouble());
     
     qCDebug(BlanketComplexItemLog) << "loadV3, loading the complex items manually";

     if (manualGrid) {
         _sprayCalc.setCameraBrand(SprayCalc::canonicalManualCameraName());
     } else {
         ...
         _sprayCalc.landscape()->setRawValue            (cameraObject[_jsonV3CameraOrientationLandscapeKey].toBool(true));
         _sprayCalc.frontalOverlap()->setRawValue       (cameraObject[_jsonV3FrontalOverlapKey].toInt());
         _sprayCalc.sideOverlap()->setRawValue          (cameraObject[_jsonV3SideOverlapKey].toInt());
         _sprayCalc.sensorWidth()->setRawValue          (cameraObject[_jsonV3CameraSensorWidthKey].toDouble());
         _sprayCalc.sensorHeight()->setRawValue         (cameraObject[_jsonV3CameraSensorHeightKey].toDouble());
         _sprayCalc.focalLength()->setRawValue          (cameraObject[_jsonV3CameraFocalLengthKey].toDouble());
         _sprayCalc.imageWidth()->setRawValue           (cameraObject[_jsonV3CameraResolutionWidthKey].toInt());
         _sprayCalc.imageHeight()->setRawValue          (cameraObject[_jsonV3CameraResolutionHeightKey].toInt());
         _sprayCalc.minTriggerInterval()->setRawValue   (cameraObject[_jsonV3CameraMinTriggerIntervalKey].toDouble(0));
         _sprayCalc.imageDensity()->setRawValue         (cameraObject[_jsonV3GroundResolutionKey].toDouble());
         _sprayCalc.fixedOrientation()->setRawValue     (false);
         _sprayCalc._setCameraNameFromV3TransectLoad    (cameraObject[_jsonV3CameraNameKey].toString());
     }
 ...
 void BlanketComplexItem::_reverseTransectOrder(QList<QList<QGeoCoordinate>>& transects)
 {
 ...
 void BlanketComplexItem::_reverseInternalTransectPoints(QList<QList<QGeoCoordinate>>& transects)
 {
 ...
 void BlanketComplexItem::_optimizeTransectsForShortestDistance(const QGeoCoordinate& distanceCoord, QList<QList<QGeoCoordinate>>& transects)
 {
 ...
 qreal BlanketComplexItem::_ccw(QPointF pt1, QPointF pt2, QPointF pt3)
 {
     return (pt2.x()-pt1.x())*(pt3.y()-pt1.y()) - (pt2.y()-pt1.y())*(pt3.x()-pt1.x());
 }
 
 qreal BlanketComplexItem::_dp(QPointF pt1, QPointF pt2)
 {
     return (pt2.x()-pt1.x())/qSqrt((pt2.x()-pt1.x())*(pt2.x()-pt1.x()) + (pt2.y()-pt1.y())*(pt2.y()-pt1.y()));
 }
 
 void BlanketComplexItem::_swapPoints(QList<QPointF>& points, int index1, int index2)
 {
 ...
 bool BlanketComplexItem::_gridAngleIsNorthSouthTransects()
 {
 ...
 void BlanketComplexItem::_adjustTransectsToEntryPointLocation(QList<QList<QGeoCoordinate>>& transects)
 {
     ...
     if (reversePoints) {
         qCDebug(BlanketComplexItemLog) << "_adjustTransectsToEntryPointLocation Reverse Points";
         _reverseInternalTransectPoints(transects);
     }
     if (reverseTransects) {
         qCDebug(BlanketComplexItemLog) << "_adjustTransectsToEntryPointLocation Reverse Transects";
         _reverseTransectOrder(transects);
     }
 
     qCDebug(BlanketComplexItemLog) << "_adjustTransectsToEntryPointLocation Modified entry point:entryLocation" << transects.first().first() << _entryPoint;
 }
 QPointF BlanketComplexItem::_rotatePoint(const QPointF& point, const QPointF& origin, double angle)
 {
     QPointF rotated;
 ...
 void BlanketComplexItem::_intersectLinesWithRect(const QList<QLineF>& lineList, const QRectF& boundRect, QList<QLineF>& resultLines)
 {
 ...
 void BlanketComplexItem::_intersectLinesWithPolygon(const QList<QLineF>& lineList, const QPolygonF& polygon, QList<QLineF>& resultLines)
 {
     resultLines.clear();
 ...
 void BlanketComplexItem::_adjustLineDirection(const QList<QLineF>& lineList, QList<QLineF>& resultLines)
 {
     qreal firstAngle = 0;
     for (int i=0; i<lineList.count(); i++) {
         const QLineF& line = lineList[i];
 ...
 double BlanketComplexItem::_clampGridAngle90(double gridAngle)
 {
 ...
 bool BlanketComplexItem::_nextTransectCoord(const QList<QGeoCoordinate>& transectPoints, int pointIndex, QGeoCoordinate& coord)
 {
 ...
 bool BlanketComplexItem::_hasTurnaround(void) const
 {
     return _turnAroundDistance() > 0;
 }
 
 double BlanketComplexItem::_turnaroundDistance(void) const
 {
     return _turnAroundDistanceFact.rawValue().toDouble();
 }
 
 void BlanketComplexItem::_rebuildTransectsPhase1(void)
 {
 ...
 void BlanketComplexItem::_rebuildTransectsPhase1WorkerSinglePolygon(bool refly)
 {
     if (_ignoreRecalc) {
         return;
     }
     ...
     qCDebug(BlanketComplexItemLog) << "_rebuildTransectsPhase1 Convert polygon to NED - _surveyAreaPolygon.count():tangentOrigin" << _surveyAreaPolygon.count() << tangentOrigin;
     for (int i=0; i<_surveyAreaPolygon.count(); i++) {
         ...
         qCDebug(BlanketComplexItemLog) << "_rebuildTransectsPhase1 vertex:x:y" << vertex << polygonPoints.last().x() << polygonPoints.last().y();
     }
 
     // Generate transects
 
     double gridAngle = _gridAngleFact.rawValue().toDouble();
     double gridSpacing = _sprayCalc.adjustedFootprintSide()->rawValue().toDouble();
     if (gridSpacing < 0.5) {
     ...
     qCDebug(BlanketComplexItemLog) << "_rebuildTransectsPhase1 Clamped grid angle" << gridAngle;
 
     qCDebug(BlanketComplexItemLog) << "_rebuildTransectsPhase1 gridSpacing:gridAngle:refly" << gridSpacing << gridAngle << refly;
 
     // Convert polygon to bounding rect
 
     qCDebug(BlanketComplexItemLog) << "_rebuildTransectsPhase1 Polygon";
     QPolygonF polygon;
     for (int i=0; i<polygonPoints.count(); i++) {
         qCDebug(BlanketComplexItemLog) << "Vertex" << polygonPoints[i];
         polygon << polygonPoints[i];
     }
     polygon << polygonPoints[0];
     QRectF boundingRect = polygon.boundingRect();
     QPointF boundingCenter = boundingRect.center();
     qCDebug(BlanketComplexItemLog) << "Bounding rect" << boundingRect.topLeft().x() << boundingRect.topLeft().y() << boundingRect.bottomRight().x() << boundingRect.bottomRight().y();
     ...
     for (const QList<QGeoCoordinate>& transect : transects) {
         QGeoCoordinate                                  coord;
         QList<SprayStyleBlanketItem::CoordInfo_t>    coordInfoTransect;
         SprayStyleBlanketItem::CoordInfo_t           coordInfo;
         ...
         if (triggerCamera() && hoverAndCaptureEnabled()) {
             ...
             if (triggerDistance() < transectLength) {
                 ...
                 qCDebug(BlanketComplexItemLog) << "cInnerHoverPoints" << cInnerHoverPoints;
                 for (int i=0; i<cInnerHoverPoints; i++) {
                     ...
                     SprayStyleBlanketItem::CoordInfo_t coordInfo = { hoverCoord, CoordTypeInteriorHoverTrigger };
         ...
         if (_hasTurnaround()) {
             ...
             SprayStyleBlanketItem::CoordInfo_t coordInfo = { turnaroundCoord, CoordTypeTurnaround };
 ...
 void BlanketComplexItem::_rebuildTransectsPhase1WorkerSplitPolygons(bool refly)
 {
     if (_ignoreRecalc) {
         return;
     }
 ...
 //        qCDebug(BlanketComplexItemLog) << "Transects from polynom p " << p;
         _rebuildTransectsFromPolygon(refly, *p, tangentOrigin, vMatch);
     }
 }
 
 void BlanketComplexItem::_PolygonDecomposeConvex(const QPolygonF& polygon, QList<QPolygonF>& decomposedPolygons)
 {
 ...
 bool BlanketComplexItem::_VertexCanSeeOther(const QPolygonF& polygon, const QPointF* vertexA, const QPointF* vertexB) {
 ...
 //    qCDebug(BlanketComplexItemLog) << "_VertexCanSeeOther false after first checks ";
 ...
 //    qCDebug(BlanketComplexItemLog) << "_VertexCanSeeOther distanceAB " << distanceAB;
 ...
 //            qCDebug(BlanketComplexItemLog) << "*vertexA " << *vertexA << "*vertexB " << *vertexB  << " intersection " << intersection;
             ...
             qCDebug(BlanketComplexItemLog) << "_VertexCanSeeOther distanceIntersection " << distanceIntersection;
 ...
 bool BlanketComplexItem::_VertexIsReflex(const QPolygonF& polygon, QList<QPointF>::const_iterator& vertexIter) {
     auto vertexBefore = vertex == polygon.begin() ? polygon.end() - 1 : vertex - 1;
     auto vertexAfter = vertex == polygon.end() - 1 ? polygon.begin() : vertex + 1;
 ...
 void BlanketComplexItem::_rebuildTransectsFromPolygon(bool refly, const QPolygonF& polygon, const QGeoCoordinate& tangentOrigin, const QPointF* const transitionPoint)
 {
     // Generate transects
     ...
     qCDebug(BlanketComplexItemLog) << "_transects.size() " << _transects.size();
 }
 
 void BlanketComplexItem::_recalcCameraShots(void)
 {
     double triggerDistance = this->triggerDistance();
     double area = _surveyAreaPolygon.area();
     double hectar = area/10000;

     if (triggerDistance == 0) {
         _cameraShots = 0;
     } else {
         if (_cameraTriggerInTurnAroundFact.rawValue().toBool()) {
             _cameraShots = qCeil(_complexDistance / (triggerDistance*hectar));
         } else {
             _cameraShots = 0;
             ...
             if (_loadedMissionItemsParent) {
                 ...
                 for (const QList<SprayStyleBlanketItem::CoordInfo_t>& transect: _transects) {
 ...
 BlanketComplexItem::ReadyForSaveState BlanketComplexItem::readyForSaveState(void) const
 {
     return SprayStyleBlanketItem::readyForSaveState();
 }
 
 void BlanketComplexItem::rotateEntryPoint(void)
 {
 ...
 double BlanketComplexItem::timeBetweenShots(void)
 {
     return _vehicleSpeed == 0 ? 0 : triggerDistance() / _vehicleSpeed;
 }
 
 double BlanketComplexItem::additionalTimeDelay (void) const
 {
     double hoverTime = 0;
 ...
 void BlanketComplexItem::_updateWizardMode(void)
 {
