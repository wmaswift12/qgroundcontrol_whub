...
Q_DECLARE_LOGGING_CATEGORY(SprayStyleBlanketItemLog)

class PlanMasterController;

class SprayStyleBlanketItem : public ComplexMissionItem
{
    Q_OBJECT

public:
    SprayStyleBlanketItem (PlanMasterController* masterController, bool flyView, QString settignsGroup);

    Q_PROPERTY(QGCMapPolygon*   surveyAreaPolygon           READ surveyAreaPolygon                                  CONSTANT)
    Q_PROPERTY(SprayCalc*       sprayCalc                   READ sprayCalc                                          CONSTANT)
    ...
    Q_PROPERTY(Fact*            gridAngle                   READ gridAngleFact                                      CONSTANT)

    Q_PROPERTY(double           totalLineDistance           READ totalLineDistance                                  NOTIFY totalLineDistanceChanged)

    //Q_PROPERTY(BlanketComplexItem*       blanketComplex     READ blanketComplex                                     CONSTANT)
    ...
    double  triggerDistance         (void) const { return _sprayCalc.adjustedFootprintFrontal()->rawValue().toDouble(); }
    ...
    double  totalLineDistance   (void) const { return _totalLineDistance; }
    ...
    QString             commandDescription          (void) const override { return tr("Transects"); }
    QString             commandName                 (void) const override { return tr("Transects"); }
...
protected:
    ...
    void    _appendScriptSprayON            (QList<MissionItem*>& items, QObject* missionItemParent, int& seqNum, float triggerDistance, int off);
    void    _appendScriptSprayOFF           (QList<MissionItem*>& items, QObject* missionItemParent, int& seqNum, float triggerDistance, int off);
    void    _appendSprayAreaON              (QList<MissionItem*>& items, QObject* missionItemParent, int& seqNum, float triggerDistance);
    void    _appendSprayAreaOFF             (QList<MissionItem*>& items, QObject* missionItemParent, int& seqNum, float triggerDistance);
    void    _appendResumeON                 (QList<MissionItem*>& items, QObject* missionItemParent, int& seqNum, MAV_FRAME mavFrame, float holdTime, const QGeoCoordinate& coordinate);
    void    _appendResumeOFF                (QList<MissionItem*>& items, QObject* missionItemParent, int& seqNum, MAV_FRAME mavFrame, float holdTime, const QGeoCoordinate& coordinate);

    void    _appendCameraTriggerDistanceUpdatePoint(QList<MissionItem*>& items, QObject* missionItemParent, int& seqNum, MAV_FRAME mavFrame, const QGeoCoordinate& coordinate, bool useConditionGate, float triggerDistance, int off);
    ...
    double          _totalLineDistance = qQNaN();

    int             _cameraShots =      0;
    double          _timeBetweenShots = 0;
    double          _vehicleSpeed =     5;
    SprayCalc      _sprayCalc;
    ...
    static constexpr const char* _jsonSprayStyleBlanketItemKey         = "SprayStyleBlanketItem";
    static constexpr const char* _jsonSprayCalcKey                     = "SprayCalc";
