...
Q_DECLARE_LOGGING_CATEGORY(SprayStyleSpotItemLog)

class PlanMasterController;

class SprayStyleSpotItem : public ComplexMissionItem
{
    Q_OBJECT

public:
    SprayStyleSpotItem(PlanMasterController* masterController, bool flyView, QString settignsGroup);

    Q_PROPERTY(QGCMapPolygon*   surveyAreaPolygon           READ surveyAreaPolygon                                  CONSTANT)
    Q_PROPERTY(SpotCalc*        spotCalc                    READ spotCalc                                           CONSTANT)
    ...
    SpotCalc*       spotCalc            (void) { return &_spotCalc; }
    QVariantList    visualTransectPoints(void) { return _visualTransectPoints; }

    Fact* turnAroundDistance            (void) { _turnAroundDistanceFact.setRawValue(0); return &_turnAroundDistanceFact; }
    ...
    double  triggerDistance         (void) const { return _spotCalc.adjustedFootprintFrontal()->rawValue().toDouble(); }
...
protected:
    ...
    void    _appendResume                   (QList<MissionItem*>& items, QObject* missionItemParent, int& seqNum, MAV_FRAME mavFrame, const QGeoCoordinate& coordinate);
    void    _appendSpraySpotON              (QList<MissionItem*>& items, QObject* missionItemParent, int& seqNum, float triggerDistance);
    void    _appendSpraySpotOFF             (QList<MissionItem*>& items, QObject* missionItemParent, int& seqNum, float triggerDistance);
    void    _appendSpraySpotUpdatePoint     (QList<MissionItem*>& items, QObject* missionItemParent, int& seqNum, MAV_FRAME mavFrame, const QGeoCoordinate& coordinate, bool useConditionGate, float triggerDistance);
    ...
    static constexpr const char* _jsonSprayStyleSpotItemKey      = "SprayStyleSpotItem";
    static constexpr const char* _jsonSpotCalcKey                    = "SpotCalc";
