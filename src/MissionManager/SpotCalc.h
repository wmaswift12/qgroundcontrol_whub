...
class SpotCalc : public CameraSpec
{
    Q_OBJECT

public:
    SpotCalc(PlanMasterController* masterController, const QString& settingsGroup, QObject* parent = nullptr);
    ...
    // When we are creating a manual grid we still use SpotCalc to store the manual grid information. It's a bastardization of what
    // SpotCalc is meant for but it greatly simplifies code and persistance of manual grids.
    ...
    Fact* adjustedFootprintSide     (void) { _adjustedFootprintSideFact.setRawValue(0); return &_adjustedFootprintSideFact; }
