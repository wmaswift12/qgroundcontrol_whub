 ...
 ****************************************************************************/
 ...
 {//Display on Plan tool bar -> "Pattern"
     ...
     if (itemName == SurveyComplexItem::name) {
        ...
        //Additional for SPOT
     } else if (itemName == SpotComplexItem::name) {
         newItem = new SpotComplexItem(_masterController, _flyView, QString() /* kmlFile */);

         //Additional for BLANKET
     } else if (itemName == BlanketComplexItem::name) {
         newItem = new BlanketComplexItem(_masterController, _flyView, QString() /* kmlFile */);
         newItem->setCoordinate(mapCenterCoordinate);
         ...
         if (globalAltitudeMode() == QGroundControlQmlGlobal::AltitudeModeMixed) {
             // We are in mixed altitude modes, so copy from previous. Otherwise alt mode will be set from global setting in constructor.
             if (_findPreviousAltitude(visualItemIndex, &prevAltitude, &prevAltMode)) {
                 qobject_cast<BlanketComplexItem*>(newItem)->sprayCalc()->setDistanceMode(prevAltMode);
             }
         }
         //END Modify
 ...
 {//Display on Plan tool bar -> "Pattern"
     ...
     //Additional for BLANKET
         ...
         newItem = new BlanketComplexItem(_masterController, _flyView, file);
     
     } else if (itemName == SpotComplexItem::name) {
         newItem = new SpotComplexItem(_masterController, _flyView, file);
     }//END Modify
     ...
     for (int i=0; i<rgMissionItems.count(); i++) {
         ...
         if (itemType == VisualMissionItem::jsonTypeSimpleItemValue) {
             ...
             if (simpleItem->load(itemObject, nextSequenceNumber, errorString)) {
                 ...
                 if (simpleItem->command() == 42702){
                    qCDebug(MissionControllerLog) << "SCRIPT_TIME: FOUND!!!";
                    if (simpleItem->missionItem().param1() == 21){
                        qCDebug(MissionControllerLog) << "SPOT DETECTED!!!";
                    }
                    else if (simpleItem->missionItem().param1() == 22){
                        qCDebug(MissionControllerLog) << "BLANKET DETECTED!!!";
                    }   
                 }
             ...
             if (complexItemType == SurveyComplexItem::jsonComplexItemTypeValue) {
                 ...
                 //Additional for BLANKET
             } else if (complexItemType == BlanketComplexItem::jsonBlanketComplexItemTypeValue) {
                 qCDebug(MissionControllerLog) << "Loading Survey: nextSequenceNumber" << nextSequenceNumber;
                 BlanketComplexItem* blanketItem = new BlanketComplexItem(_masterController, _flyView, QString() /* kmlFile */);
                 if (!blanketItem->load(itemObject, nextSequenceNumber++, errorString)) {
                     return false;
                 }
                 nextSequenceNumber = blanketItem->lastSequenceNumber() + 1;
                 qCDebug(MissionControllerLog) << "Spot load complete: nextSequenceNumber" << nextSequenceNumber;
                 visualItems->append(blanketItem);
                 //END Modify

                 //Additional for SPOT
             } else if (complexItemType == SpotComplexItem::jsonSpotComplexItemTypeValue) {
                 qCDebug(MissionControllerLog) << "Loading Corridor Scan: nextSequenceNumber" << nextSequenceNumber;
                 SpotComplexItem* spotItem = new SpotComplexItem(_masterController, _flyView, QString() /* kmlFile */);
                 if (!spotItem->load(itemObject, nextSequenceNumber++, errorString)) {
                     return false;
                 }
                 nextSequenceNumber = spotItem->lastSequenceNumber() + 1;
                 qCDebug(MissionControllerLog) << "Corridor Scan load complete: nextSequenceNumber" << nextSequenceNumber;
                 visualItems->append(spotItem);
                 //END Modify
 ...
 {
     //Displaying the Button on the UI to ACTUALLY APPEAR
     QStringList complexItems;
    
     complexItems.append(BlanketComplexItem::name);
     complexItems.append(SpotComplexItem::name);
